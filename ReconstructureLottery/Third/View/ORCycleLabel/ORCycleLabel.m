...
- (void)labelChange {
    ...
    if (_width - maxX1 >= _interval || (frame2.origin.x > -_textWidth && maxX2+1 < _width + _textWidth)) {
        frame2.origin.x -= _distance;
    }
    
    if (_width - maxX2 >= _interval || (frame1.origin.x > -_textWidth && maxX1+1 < _width + _textWidth)) {
...
//        }];
        _timer = [NSTimer scheduledTimerWithTimeInterval:0.1 target:self selector:@selector(labelChange) userInfo:nil repeats:YES];
        return;
    }
